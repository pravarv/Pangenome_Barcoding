    ...
    for file in os.listdir(folder):
        ...
        if os.path.isfile(file_path):
            for record in SeqIO.parse(file_path, file_format):
                genome_dict[record.id] = str(record.seq)[:5000]
                break
    return genome_dict
...
# kmer_array, kmer_index = get_kmer_index(genome_dict,3)
...
def run_full_alg_2(input,k):
    ...
    for key in unique_kmers:
        print(f'{key} : {len(unique_kmers[key])}, {len(get_rev_com_kmers(unique_kmers[key]))}')

    su_kmers = get_strongly_unique_kmers(unique_kmers,kmer_array)
    for key in su_kmers:
        print(f'{key} : {len(su_kmers[key])}, {len(get_rev_com_kmers(su_kmers[key]))}')
...
#     start = time.time()
#     strongly_unique_kmers = run_full_alg_2(sample_genomes,10)
#     end = time.time()
#     print(end-start)
